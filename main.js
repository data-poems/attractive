    ...
    function initThemeBar() {
      ...
      if (!styleStrip || !colorStrip) return;

      // Populate style strip
      Object.values(VISUAL_STYLES).forEach(style => {
        const btn = document.createElement('button');
        btn.className = 'style-option' + (style.id === currentStyle ? ' active' : '');
        btn.dataset.style = style.id;
        btn.setAttribute('role', 'option');
        btn.setAttribute('aria-selected', style.id === currentStyle);
        btn.title = `${style.name}: ${style.description}`;
        btn.innerHTML = `<span>${style.name.substring(0, 4)}</span>`;
        btn.onclick = () => {
          selectStyle(style.id);
          // Update theme bar active state
          styleStrip.querySelectorAll('.style-option').forEach(s => {
            s.classList.toggle('active', s.dataset.style === style.id);
            s.setAttribute('aria-selected', s.dataset.style === style.id);
          });
        };
        styleStrip.appendChild(btn);
      });

      // Populate color strip
      Object.entries(colorSchemes).forEach(([id, scheme]) => {
        const btn = document.createElement('button');
        btn.className = 'color-swatch' + (id === currentColorScheme ? ' active' : '');
        btn.dataset.scheme = id;
        btn.dataset.category = scheme.category;
        btn.setAttribute('role', 'option');
        btn.setAttribute('aria-selected', id === currentColorScheme);
        btn.title = scheme.name;
        btn.style.background = `linear-gradient(135deg, rgb(${scheme.start.join(',')}), rgb(${scheme.end.join(',')}))`;
        btn.onclick = () => {
          selectColorScheme(btn);
          // Update theme bar active state
          colorStrip.querySelectorAll('.color-swatch').forEach(s => {
            s.classList.toggle('active', s.dataset.scheme === id);
            s.setAttribute('aria-selected', s.dataset.scheme === id);
          });
          // Also update sidebar if visible
          document.querySelectorAll('.color-option').forEach(opt => {
            opt.classList.toggle('active', opt.dataset.scheme === id);
            opt.setAttribute('aria-checked', opt.dataset.scheme === id);
          });
        };
      ...
      toggle.addEventListener('click', () => {
        ...
        themeBar.classList.toggle('collapsed', isExpanded);
      });
      ...
      document.querySelectorAll('.color-filter-chip').forEach(chip => {
        chip.addEventListener('click', () => {
          ...
          document.querySelectorAll('.color-filter-chip').forEach(c => {
            ...
            c.setAttribute('aria-selected', c.dataset.category === category);
          });

          // Filter color swatches
          colorStrip.querySelectorAll('.color-swatch').forEach(swatch => {
    ...
    // Sync theme bar with sidebar selections
    function syncThemeBarSelection() {
      // Sync style
      document.querySelectorAll('#styleStrip .style-option').forEach(s => {
        s.classList.toggle('active', s.dataset.style === currentStyle);
        s.setAttribute('aria-selected', s.dataset.style === currentStyle);
      });
      // Sync color
      document.querySelectorAll('#colorStrip .color-swatch').forEach(s => {
        s.classList.toggle('active', s.dataset.scheme === currentColorScheme);
        s.setAttribute('aria-selected', s.dataset.scheme === currentColorScheme);
      });
    }
